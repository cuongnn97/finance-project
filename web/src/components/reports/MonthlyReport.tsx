...
export function MonthlyReport({ year, month }: MonthlyReportProps) {
  ...
  if (isLoading) {
    return (
      <div className="flex h-64 items-center justify-center">
        <Spinner label="Đang tạo báo cáo…" />
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          ...
          },
          {
            label: "Số dư ròng",
            value: data.balance,
            color: data.balance >= 0 ? "text-gray-900" : "text-red-600",
            bg: "bg-gray-50",
          },
        ].map((s) => (
      ...
      {isEmpty ? (
        <EmptyState title="Chưa có giao dịch tháng này" />
      ) : (
        <>
            ...
            <Card>
              ...
              <ul className="space-y-2">
                {data.income_by_category.map((c) => (
                  <li
                    key={c.category_id ?? "other"}
                    className="flex items-center gap-3"
                  >
                    <div
                      className="h-2.5 w-2.5 rounded-full"
                      style={{ backgroundColor: c.category_color ?? "#d1d5db" }}
                    />
